...
@RestController
@ExposesResourceFor(Mensagem.class)
...
public class MessageController {
    ...
    public ResponseEntity<List<MessageResource>> messages(@RequestHeader(value = "Version", required = false) String version) {
        List<Mensagem> mensagens = SensorDAO.obterMessages();
        List<MessageResource> resource = resourceAssembler.toResources(mensagens);
    ...
    public ResponseEntity<MessageResource> message(@PathVariable("id") int id) {
        Mensagem mensagem = SensorDAO.obterMessage(id);
        MessageResource resource = resourceAssembler.toResource(mensagem);
